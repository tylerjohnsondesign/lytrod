...
<script id="tmpl-nf-cl-advanced-condition" type="text/template">
    ...
    <div class="nf-condition">
        <div class="nf-first-when-container">
            <div class="nf-condition-header">
                <div class="nf-condition-drag-handle">
                    <i class="fa fa-bars nf-drag-condition" aria-hidden="true" title="{{{ nfcli18n.dragToReorder }}}"></i>
                </div>
                <div class="nf-condition-label-input">
                    <input type="text" class="setting nf-condition-label-field" data-id="label" aria-label="{{{ nfcli18n.conditionLabelPlaceholder }}}" placeholder="{{{ nfcli18n.conditionLabelPlaceholder }}}" value="{{{ data.label }}}" />
                </div>
                <div class="nf-when-controls">
                    <button type="button" class="nf-collapse-condition nf-icon-btn" aria-label="{{{ ( data.collapsed ) ? nfcli18n.expandCondition : nfcli18n.collapseCondition }}}" aria-expanded="{{{ ( data.collapsed ) ? 'false' : 'true' }}}">
                        <i class="fa fa-chevron-circle-{{{ ( data.collapsed ) ? 'down' : 'up' }}}" aria-hidden="true"></i>
                    </button>
                    <button type="button" class="nf-remove-condition nf-icon-btn" aria-label="{{{ nfcli18n.deleteConditionTitle }}}">
                        <i class="fa fa-trash" aria-hidden="true"></i>
                    </button>
                </div>
            </div>
            <div class="nf-condition-when-text">
        ...
        <div class="nf-when-container">
            ...
            <# if ( ! data.collapsed ) { #>
                <button type="button" class="nf-add-when nf-icon-btn" aria-label="{{{ nfcli18n.addWhenClause }}}">
        ...
        <# if ( ! data.collapsed ) { #>
            <div class="nf-then-container">
                ...
                <button type="button" class="nf-add-then nf-icon-btn" aria-label="{{{ nfcli18n.addThenClause }}}">
                    <i class="fa fa-plus-circle" aria-hidden="true"></i>
                </button>
            </div>
            ...
            <div class="nf-else-container">
                ...
                <button type="button" class="nf-add-else nf-icon-btn" aria-label="{{{ nfcli18n.addElseClause }}}">
                    <i class="fa fa-plus-circle" aria-hidden="true"></i>
                </button>
            </div>
            <# } #>
...
<script id="tmpl-nf-cl-advanced-when-item" type="text/template">
    <div class="nf-when">
        <div class="nf-setting nf-one-fourth">
            <button type="button" class="nf-remove-when nf-icon-btn" aria-label="{{{ nfcli18n.removeWhenClause }}}">
...
<script id="tmpl-nf-cl-trigger-item" type="text/template">
    <div class="nf-{{{ data.modelType }}}">
        <div class="nf-setting nf-one-fourth">
            <button type="button" class="nf-remove-{{{ data.modelType }}} nf-icon-btn" aria-label="{{{ ( 'then' === data.modelType ) ? nfcli18n.removeThenClause : nfcli18n.removeElseClause }}}">
                <i class="fa fa-minus-circle" aria-hidden="true"></i>
            </button>
        </div>
        <div class="nf-setting nf-one-fourth">
