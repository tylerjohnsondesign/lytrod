...
define( 'controllers/templateHelpers',[], function() {
	var controller = Marionette.Object.extend( {
		initialize: function() {
			...
			nfRadio.channel('conditions-recaptcha').reply( 'get:triggers', this.recaptchaTriggers );
			nfRadio.channel('conditions-turnstile').reply( 'get:triggers', this.turnstileTriggers );
		...
		renderKeySelect: function( currentValue, modelType ) {
			...
			var fieldOptions = _.chain( fieldCollection.models )
				...
				.filter( function( field )  {
					...
					var notForWhen = [ 'submit', 'hr', 'html', 'save', 'file-upload', 'password', 'passwordconfirm', 'product', 'recaptcha', 'turnstile' ];
		...
		recaptchaTriggers: function( triggers ) {
			delete triggers.change_value;
			delete triggers.set_required;
			delete triggers.unset_required;
			return triggers;
		},

		turnstileTriggers: function( triggers ) {
			delete triggers.change_value;
			delete triggers.set_required;
			delete triggers.unset_required;
			return triggers;
 ...
 * @since 3.0
 */
define( 'controllers/conditionSorting',[], function() {
	var controller = Marionette.Object.extend({
		initialize: function() {
			/*
