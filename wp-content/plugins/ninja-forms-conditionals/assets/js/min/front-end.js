	...
	var model = Backbone.Model.extend( {
		...
		maybeupdateFieldCompare: function( el, fieldModel, keyCode ) {
			if( 'checkbox' == fieldModel.get( 'type' ) ){
				...
				/*
				 * Use the field model's actual selected value rather than jQuery(el).val().
				 * jQuery(el).val() returns the value attribute of whichever radio button has
				 * focus, even if it has not been selected by the user. When tabbing into a
				 * radio field, the keyup event fires on the focused (but unselected) radio
				 * button, causing jQuery(el).val() to return that option's value and
				 * incorrectly trigger conditions. The field model value only reflects a
...
define( 'models/conditionModel',[ 'models/whenCollection' ], function( WhenCollection ) {
	var model = Backbone.Model.extend( {
		initialize: function( options ) {
			 ...
			 * Deferred via setTimeout to allow all field relationships (e.g. Confirm field pairings)
			 * to be fully established before conditions first evaluate on page load.
			 */
			setTimeout( function() {
				this.checkWhen();
			}.bind( this ), 0 );
