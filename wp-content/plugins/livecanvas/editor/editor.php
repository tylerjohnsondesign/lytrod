...
// to limit the UI kit choice
//define('LC_EDITOR_DISABLE_UIKIT_CHANGE', TRUE);

// OPENAI KEY FILTERING EXAMPLE
/*
function modify_lc_openai_key($openai_key) {
    return 'xxx';
}
add_filter('filter_lc_openai_key', 'modify_lc_openai_key');
*/
...
<html data-active-plugins="<?php echo lc_get_active_plugins_list() ?>">
	<head>
		...
		<script id="livecanvas-environment-variables">
			...
			lc_editor_plugin_version='<?php echo LC_SCRIPTS_VERSION ?>'; //still unused
			lc_editor_fragment_type='<?php  if ( get_post_meta($post->ID,'is_header',TRUE)!='' ) echo "header"; 	if ( get_post_meta($post->ID,'is_footer',TRUE)!='' ) echo "footer"; ?>'; //still unused
			lc_editor_root_url='<?php lc_print_editor_url() ?>';
			lc_editor_saving_url='<?php echo admin_url( 'admin-ajax.php' ) ?>';
			lc_editor_saving_nonce='<?php echo wp_create_nonce('lc_save_page'); ?>';
			...
			lc_editor_url_before_editor="<?php if (isset($_GET['from_url'])) echo remove_query_arg( array('lc_import_success'  ), urldecode($_GET['from_url']));
                /*
				if(isset($_GET['demo_id'])){ echo get_permalink($_GET['demo_id']); } else { echo (   (isset($_GET['from_page_edit'])) ? admin_url( 'post.php' )."?action=edit&post=" . $post->ID : get_permalink($post->ID)   ); }
                */
			?>";
            lc_editor_child_theme_uri = "<?php echo esc_js(get_stylesheet_directory_uri()) ?>";  
			lc_editor_url_to_load="<?php $current_url = "//$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"; $current_url=remove_query_arg( array('lc_action_launch_editing'  ), $current_url );	$current_url=remove_query_arg( array('lc_import_success'  ), $current_url ); $current_url=add_query_arg(  'lc_page_editing_mode','1', $current_url );	echo $current_url;	?>" + "&lc_random=" + Math.floor(Math.random() * 1000);
			lc_editor_apikey="<?php echo esc_js(lc_get_apikey()) ?>";
			...
			lc_editor_hide_readymade_pages=<?php if (   /*  !current_user_can("administrator") &&  */  lc_plugin_option_is_set("hide-readymade-pages")) echo "true"; else echo "false"; ?>;
            ...
            lc_editor_load_theme_uikit=<?php echo (lc_theme_has_custom_html('sections')) ? "true" : "false"; ?>;
            lc_editor_disable_uikit_change=<?php echo (defined('LC_EDITOR_DISABLE_UIKIT_CHANGE') && LC_EDITOR_DISABLE_UIKIT_CHANGE) ? "true" : "false"; ?>;    
            lc_editor_openai_key='<?php echo esc_js(apply_filters('filter_lc_openai_key', get_option('lc_openai_key'))); ?>';
            lc_editor_openrouter_key='<?php echo esc_js(apply_filters('filter_lc_openrouter_key', get_option('lc_openrouter_key'))); ?>'; 
            lc_editor_website_info='<?php echo esc_js(get_option('lc_website_info')) ?>';  
            lc_editor_site_id='<?php echo esc_js(function_exists('get_current_blog_id') ? get_current_blog_id() : 1) ?>';  
			lc_editor_whitelabel=<?php if ( lc_plugin_option_is_set("whitelabel")) echo "true"; else echo "false"; ?>;
			lc_editor_custom_tags = <?php echo json_encode(lc_get_custom_elements_registry()) ?>;
        </script>


		<script type='text/javascript' src="https://cdn.livecanvas.com/remote/lc-bundle-001-g87r37g84j2312hve6xx2-w-v4.js"></script>
        ...
        <?php if (!empty(locate_template('livecanvas/editor-configuration.js')) ){   ?>
                ...
                <?php } else {  ?>
                <script data-pagespeed-no-defer type='text/javascript' src='<?php lc_print_editor_url() ?>configs/<?php echo lc_get_framework_slug(); ?>.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script>
		...
		<script type='text/javascript' src='<?php lc_print_editor_url() ?>configs/editor-api.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script>
		    
		
        <script type='text/javascript' src='<?php lc_print_editor_url() ?>functions.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script>
        <script type='text/javascript' src='<?php lc_print_editor_url() ?>db-service.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script>   
        ...
        <script type='text/javascript' src='<?php lc_print_editor_url() ?>live-editing-initialize.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script>
             

		<script type='text/javascript' src='<?php lc_print_editor_url() ?>editor.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script> 
		<script type='text/javascript' src='<?php lc_print_editor_url() ?>text-editing-toolbar.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script> 
		<script type='text/javascript' src='<?php lc_print_editor_url() ?>side-panels/edit-properties.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script>
		 
        <script defer type='text/javascript' src='<?php lc_print_editor_url() ?>tree-view.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script> 
        
        <script defer type='text/javascript' src='<?php lc_print_editor_url() ?>side-panels/ai-assistant.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script> 
        <script defer type='text/javascript' src='<?php lc_print_editor_url() ?>side-panels/ai-screenshot-to-code.js?v=<?php echo LC_SCRIPTS_VERSION ?>'></script> 
	...
	<body>
        ...
        <section id="readymades-modal-wrapper" style="display:none">
            <?php include('modal-readymade-pages.html'); ?> 
        </section>
		...
		<div id="sidepanel" hidden>
			...
			<div class="top-actions">
				<!-- <a class="minimize-sidepanel"> <i class="fa fa-chevron-down"></i></a> -->
				<!-- <a class="toggle-side-mode"> <i class="fa fa-chevron-circle-left"></i></a> -->
				<a class="sidepanel-action-edit-html" alt="Open Code Editor">
					<svg width="21" height="21" fill="var(--color-grey-2)" viewBox="0 0 16 16" class="bi bi-code-slash"
						xmlns="http://www.w3.org/2000/svg">
						<path fill-rule="evenodd" d="M4.854 4.146a.5.5 0 0 1 0 .708L1.707 8l3.147 3.146a.5.5 0 0 1-.708.708l-3.5-3.5a.5.5 0 0 1 0-.708l3.5-3.5a.5.5 0 0 1 .708 0zm6.292 0a.5.5 0 0 0 0 .708L14.293 8l-3.147 3.146a.5.5 0 0 0 .708.708l3.5-3.5a.5.5 0 0 0 0-.708l-3.5-3.5a.5.5 0 0 0-.708 0zm-.999-3.124a.5.5 0 0 1 .33.625l-4 13a.5.5 0 0 1-.955-.294l4-13a.5.5 0 0 1 .625-.33z">
						</path>
					</svg>
				</a> &nbsp;&nbsp;
				<a class="close-sidepanel">
					<svg width="21" height="21" fill="var(--color-grey-2)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
						<path fill="color: var(--color-grey-2);" d="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z" />
					</svg>
				</a>
			</div>

			<section item-type="generic">
				<h1>Item x</h1>
				<form>
				</form>
			</section>
			...
			<!-- welcome panel -->
			<?php include('side-panels/welcome.html'); ?>
			<?php include('side-panels/welcome-whitelabel.html'); ?>

			<!-- abstract editing panel for any html element -->
			<?php include('side-panels/edit-properties.html'); ?>

			<!-- content editing -->
			<?php include('side-panels/add-row.html'); ?>
			<?php include('side-panels/ai-assistant.html'); ?>
			<?php include('side-panels/blocks.html'); ?>
			<?php include('side-panels/background.html'); ?>
			<?php include('side-panels/button.html'); ?>
			<?php include('side-panels/gmap-embed.html'); ?>
			<?php include('side-panels/history.html'); ?>
			<?php include('side-panels/icon.html'); ?>
			<?php include('side-panels/image.html'); ?>
			<?php include('side-panels/shortcode.html'); ?>
			<?php include('side-panels/svg-icon.html'); ?>
			<?php include('side-panels/posts-loop.html'); ?>
			<?php include('side-panels/tangible-loop.html'); ?>
			<?php include('side-panels/twig.html'); ?>
			<?php include('side-panels/video-bg.html'); ?>
			<?php include('side-panels/video-embed.html'); ?>

			<?php include('side-panels/sections.html'); ?>


			<?php do_action( 'lc_render_additional_panels' ); ?>

			 
		</div> <!-- close sidepanel -->
		
		
		<?php include('side-panels/side-panel-templates.html'); ?> 
		...
		<div id="previewiframe-wrap" class="push-aside-preview">
		...
		<section><?php include('empty-section.html'); ?></section>
		
		<section id="lc-code-editor-window"><?php include('code-editor.html'); ?></section>		
		
		<section id="lc-modal-link" class="lc-modal">
			<?php include ('modal-link.html'); ?>
		</section>

        <section id="lc-modal-save-snippet" class="lc-modal">
			<?php include ('modal-save-snippet.html'); ?>
		</section>
