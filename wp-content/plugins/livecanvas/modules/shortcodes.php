...
function lc_get_posts_func( $atts, $content = null ){
    ...
    // If content is provided, use it as a template
    if ($content && $output_view == 'lc_get_posts_default_view') {
        $output = '';
        global $post;
        $original_post = $post; // Store the original post
        foreach ($the_posts as $post) {
            setup_postdata($post); 
            $output .= lct_do_shortcode(lc_strip_lc_attributes($content));
        }
        wp_reset_postdata();
        $post = $original_post; // Restore the original post
        return $output;
    }
...
add_shortcode( 'lc_get_post', function  ( $atts ){
    ...
    $processed_content = lct_do_shortcode( 
        lc_strip_lc_attributes( 
            lc_process_custom_elements( $the_post->post_content )
        )
    );

	//if class attribute is provided, filter output 
	if (isset($atts['class'])) $processed_content = lc_filter_html_by_class($atts['class'], $processed_content);
...
// SHORTCODE TO GET HTML PARTIALS FROM DB OR FILE /////
add_shortcode('lc_get_partial', function ($atts) {
    ...
    if (!$slug && !$file) {
        return "<div><code><h2>Error in <i>lc_get_partial</i> shortcode </h2> <p>You need to specify the <b>slug</b> or the <b>file</b> parameter.</p></code></div>";
    }

    if ($slug){
		$the_post = lc_get_post_by_slug($slug, $post_type);
        if (!$the_post) {
            return "<div><code><h2>Error in <i>lc_get_partial</i> shortcode </h2> <p>Could not retrieve any <b>$post_type </b> post with slug <b> $slug </b></p> </code></div>";
        }
        $template_content = $the_post -> post_content;
    }

    if($file){
        $file_path = get_stylesheet_directory() . '/partials/' . sanitize_file_name($file) . '.html';
        if (!file_exists($file_path)) {
            return "<div><code><h2>Error in <i>lc_get_partial</i> shortcode </h2> <p>Could not find file: <b>'{$file}.html'</b> in the /partials/ folder of your active (child) theme. </p></code></div>";
        }
...
//DYNAMIC CONDITIONAL SHORTCODE: lc_if
//[lc_if condition="is_user_logged_in" param="some_value"] Content to show if is_user_logged_in is true. [lc_else] Content to show if  is_user_logged_in is false. [/lc_if]

add_shortcode( 'lc_if', function( $atts, $content = null ) {
    // Extract shortcode attributes
    $atts = shortcode_atts(
        array(
            'condition' => '', // Condition to check (e.g., is_paged)
    ...
    if ( empty( $atts['condition'] ) ) {
        return 'Error: Please provide a condition.';
    }

    // Trim the condition name
    $condition_function = trim( $atts['condition'] );

    // Determine if condition should be negated
    $negate = false;
    if ( strpos( $condition_function, '!' ) === 0 ) {
        $negate = true;
        // Strip the '!' from the function name
        $condition_function = substr( $condition_function, 1 );
    }

    // Check if the function exists
    if ( ! function_exists( $condition_function ) ) {
        return 'Error: Invalid condition function.';
    }

    // Evaluate the condition with optional parameter
    $param = !empty($atts['param']) ? $atts['param'] : null;
    $condition_result = $param ? call_user_func( $condition_function, $param ) : call_user_func( $condition_function );

    // Negate the condition if needed
    if ( $negate ) {
        $condition_result = ! $condition_result;
    }

    // Split content into if and else parts
    $parts = preg_split('/(\[lc_else\])/i', $content);

    // Return the appropriate content based on the condition
    if ( $condition_result ) {
        return lc_process_nested_shortcodes( isset($parts[0]) ? $parts[0] : '' );
    } else {
        return lc_process_nested_shortcodes( isset($parts[1]) ? $parts[1] : '' );
    }
});

// Function to process nested shortcodes
function lc_process_nested_shortcodes( $content ) {
    // Ensure all shortcodes are processed, including nested ones
    return do_shortcode( shortcode_unautop( $content ) );
}

// Register the closing shortcode to ensure proper parsing
add_shortcode( 'lc_else', function( $atts, $content = null ) {
    return ''; // This is a placeholder, actual logic is handled in 'lc_if'
});


//SHORTCODE lc_the_option
