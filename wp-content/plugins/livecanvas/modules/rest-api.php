...
function lc_get_all_posts_by_custom_tax($request) { 
    ...
    foreach ($all_posts_ids as $post_id) {
        ...
        //if post is js, skip
        if (get_the_title($post_id)=='Global JavaScript Code') continue;
...
function lc_get_html_files_from_theme($request) {
    ...
    $requested_folder = sanitize_text_field($request['folder']);

    // Build old path version eg picostrap/template-livecanvas-blocks
    $base_directory_path = get_stylesheet_directory() . '/' . $requested_folder; //old folder naming

    // Build new path version, eg  picostrap/livecanvas/blocks
    $new_folder_name = str_replace('template-livecanvas-', '', $requested_folder); //eliminate vintage "prefix"
    $new_folder_name = str_replace('readymade-pages','pages', $new_folder_name); //shorten el

    $livecanvas_path = get_stylesheet_directory() . '/livecanvas/' . $new_folder_name;

    // Use the new path version if it exists
    if (is_dir($livecanvas_path)) {
        $base_directory_path = $livecanvas_path; 
    }

    //optional
    // if base_directory_path still not exists, last resort is to get from plugin modules/from-picostrap/livecanvas
    //if (!is_dir($base_directory_path ) OR isset($_GET['src_plugin'])) {
    //    $base_directory_path = plugin_dir_path( __FILE__ ) . 'from-picostrap/livecanvas/' . str_replace('template-livecanvas-','',$requested_folder);
    //}

    // Finally, check if the specified folder exists within the child theme directory
    if (!is_dir($base_directory_path)) {
        return new WP_REST_Response([
            'error' => 'folder_not_found',
            'failed_base_directory_path' => $base_directory_path
        ], 406);
    }
