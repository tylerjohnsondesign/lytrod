...
function lc_print_template_assign_checkbox($key, $label){
    ...
    if (strpos($label, 'Tangible') !== false) return;
...
function lc_build_meta_boxes_template_assign( $post ){
	...
	<div class='inside' id='lc-template-assignment'>
		...
		<label class='lc-dt-label'>    
			...
			<span class="lc-pro-mode-label">View All</span> 
		</label> 
	...
	add_action('admin_footer', function(){ ?>
		...
		<script>
			...
			jQuery(document).ready(function($) {
				...
				$("body").on("click", "#lca_meta_boxes_template_assign input[type=checkbox]:not(#lc-pro-mode)", function(e) {
					...
					if ($("#lca_meta_boxes_template_assign input[type=checkbox]:checked:not(#lc-pro-mode)").length > 1) return;
