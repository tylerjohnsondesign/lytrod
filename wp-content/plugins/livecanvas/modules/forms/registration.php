...
function  lc_user_registration_action_callback() {
    ...
    */
    ...
    //CHECK USERNAME 
    if (empty($_POST['ufield'])  ) $_POST['ufield'] = $_POST['efield']; //so we allow forms with no username; email is used instead
    if (strlen ($_POST['ufield']) < 5 OR strlen ($_POST['ufield'])>40) lc_form_die("Invalid <b>Username</b>. Minimum 5 chars. Maximum 40 chars. Thank you!");
    if (!validate_username($_POST['ufield']) && !is_email($_POST['ufield']) ) lc_form_die("Invalid <b>Username</b>. Lowercase chars / numbers only, please.  ");
