...
add_shortcode('lc_form', function ($atts) {
	...
	document.currentScript.closest("form").addEventListener("submit", function (event) {
		...
		.then(response => {
			...
			// Execute any script tags in the response (useful to reset the form)
			theForm.parentElement.querySelector(".lc_form_feedback").querySelectorAll("script").forEach(script => {
