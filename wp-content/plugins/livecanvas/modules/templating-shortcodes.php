...
add_shortcode( 'lc_the_author', function( $atts ) {	
    ...
    // Apply 'the_author_meta' filter 
    $author_meta = apply_filters('the_author_meta', $author_meta, $attributes['field'], $author_id);

    if ($attributes['field']=='description') {
...
add_shortcode( 'lc_the_thumbnail', function($atts){
	...
	$attributes = shortcode_atts( array(
		...
		'class' => 'img-fluid',
		'style' => '',
        'loading' => 'lazy', //TO DOCUMENT: set parameter to '' in order to disable lazy loading
		'placeholder' => 1,
		'placeholder_url' => 'https://placehold.co/600x400?text=Placeholder'
	), $atts );

	global $post;

	$out = get_the_post_thumbnail ($post, 
                                $attributes['size'],  
                                array( 'class' => $attributes['class'], 'style' => $attributes['style'],  'loading' => $attributes['loading']  ) );

    //edge case no thumbnail available: return a placeholder
	if (empty( $out ) && $attributes['placeholder'] != 0){
		$out = '<img loading="lazy" class="'.$attributes['class'].'" style="'.$attributes['style'].'" src="'.$attributes['placeholder_url'].'" alt="" />';	
    }
...
add_shortcode( 'lc_the_archive_title', function( $atts ) {
    ...
    //search page
    if (is_search()) return  ('<h1> Search Results for:  <span class="bg-light text-dark ms-4 px-4 py-1"> <em>' . get_search_query() . '</em></span> </h1>');
...
//SHORTCODE TO GET CATEGORY METADATA  
add_shortcode( 'lc_the_archive_meta', function  ( $atts ) {
    // Extract shortcode attributes
    $atts = shortcode_atts( array(
    'field' => '', // Specify the desired metadata field
    'filter' => '' //optional filter to apply, eg the_content
    ), $atts );

    //Get term data. are we in the LC editor context or in the frontend?
    if(defined('LC_DOING_DYNAMIC_TEMPLATE_SHORTCODE_RENDERING')) {
        return "[meta placeholder]"; //GENERAL
    }  else {
        $term = get_queried_object();
    }
    // Check if term object exists and metadata field is specified
    if ( $term && $atts['field'] ) {
        // Get term meta data
        $meta_value = get_term_meta( $term->term_id, $atts['field'], true );
        if ($atts['filter']!='') $meta_value = apply_filters(  $atts['filter'], $meta_value);  
        return   $meta_value;
    } else {
        return '<!-- Empty field -->'; // Return empty string if term or field is missing
    }
});
