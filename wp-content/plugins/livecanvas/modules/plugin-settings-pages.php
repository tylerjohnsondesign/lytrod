...
function lc_main_options_page(){
    ...
    add_submenu_page('livecanvas', // Parent slug
		'AI Integration', // Page title
		'AI Integration', // Menu title
		'manage_options', // Capability
		'livecanvas_integration', // Slug
		'lc_ai_settings_page'	// Function
    );
    /*
    add_submenu_page('livecanvas', // Parent slug
		'Starter Themes', // Page title
		'Starter Themes', // Menu title
		'manage_options', // Capability
		'livecanvas_starter_themes', // Slug
		'lc_starter_themes_page'	// Function
    );
	*/
}
		...
		<form method="post">
			...
			<table id="lc-settings-table">
				...
				<tr>
					...
					<td>
						 <label> 	<input name="gtblocks" type="checkbox" value="1" <?php if (isset($lc_settings['gtblocks'])) echo "checked"; ?> > Adds admin UI to craft  Gutenberg - based "partials" to embed in a LiveCanvas page  </label> 
					</td>
				</tr>
				...
				<tr>
					<th scope="row">Hide Built-in Readymade Pages </th>
					<td>
						  <label>    
								<input name="hide-readymade-pages" type="checkbox" value="1" <?php if (isset($lc_settings['hide-readymade-pages'])) echo "checked"; ?> >
								Upon creation of a new LC page, readymade pages will not be proposed, and menu item for readymade pages is removed.
						 </label> 
					</td>
				</tr>
				<tr>
					...
					<td>
						  <label>    
								<input name="disable-ob-handling" type="checkbox" value="1" <?php if (isset($lc_settings['disable-ob-handling'])) echo "checked"; ?> >
                           		Can help in peculiar PHP environments that crash the editor. Could break compatibility with some optimization plugins. Generally leave unchecked
						 </label> 
					</td>
				</tr>
