...
add_action("wp_head", function(){
	?>
	<script>
		alert("<?php echo lc_get_parent_theme_info("Name"). " ". lc_get_parent_theme_info("Version"); ?>"); 
	</script>
	<?php 
});
*/
...
add_filter('lc_settings', function ($lc_settings) {
	if(is_array($lc_settings) && in_array(lc_get_parent_theme_info( 'Name' ), array("bootScore","Bootscore", "b5st")))   {
...
add_action("wp_head", function(){
	...
	if ( !isset($_GET["lc_page_editing_mode"]) OR !current_user_can("edit_pages") ) return; //only when page is being edited by LC
 	...
 	if(lc_get_parent_theme_info( 'Name' ) ==  "bootScore" or lc_get_parent_theme_info( 'Name' ) ==  "Bootscore" ) { 
...
//FOR CLASSICPRESS compatibility with Tangible Templating Library
if (!function_exists('excerpt_remove_blocks')):
    function excerpt_remove_blocks() {}
endif;
