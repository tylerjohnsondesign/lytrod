...
if ( ! isset( $raw ) ) {
	$custom_descp ='If you want to create dynamic directories, you can use various shortcodes:';
	$custom_descp .= '<ul>';
	...
	$custom_descp .= '</ul>';
	$custom_descp .= '<p>' . 'For Example: <code>/%formtitle%/%month%/%year%/</code> would be <code>/MyFormTitle/04/2012/</code>' . '</p>';
...
return apply_filters( 'ninja_forms_uploads_settings_upload', array(
	'max_filesize' => array(
		...
		'desc'    => 'This limit can be overridden in the settings for a File Upload field. Very large uploads are possible, but we recommend setting a sensible limit.',
	),
