...
class NF_FU_AJAX_Controllers_Uploads extends NF_Abstracts_Controller {
	...
	protected function _process() {
		foreach ( $this->_data['files'] as $key => $file ) {
			...
			if ( ! empty( $new_tmp_name ) ) {
				...
				// Validate destination extension against whitelist to prevent extension bypass via rename
				if ( ! $this->validate_extension_whitelist( $dest_extension ) ) {
					unset( $this->_data['files'][ $key ] );
					@unlink( $file['tmp_name'] );
					continue;
				}
	...
	public function get_allowed_file_types($want = "string") {
		...
		if($want = "array"){
			...
			foreach($string_list as $type) {
				...
				$upload_types_allowed[strtolower($type)] = strtolower($type);
			}
	...
	public static function check_filetype($filename, $types_list) {
		//Remove dot from ext when found and ensure lowercase
		foreach($types_list as $key => $value){
			...
			$types_list[strtolower($key)] = strtolower($value);
		}
