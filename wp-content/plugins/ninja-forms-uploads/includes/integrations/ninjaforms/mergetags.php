...
class NF_FU_Integrations_NinjaForms_MergeTags {
	...
	public function merge_tag_value( $value, $field ) {
		...
		// Reconstruct files array from field value if missing (e.g., during email retriggering)
		// Field value from database is array(upload_id => file_url) but merge tags need
		// the files array structure with upload IDs
		if ( ! isset( $field['files'] ) || empty( $field['files'] ) ) {
			...
			if ( is_string( $field_value ) && ! empty( $field_value ) && is_serialized( $field_value ) ) {
				$unserialized = unserialize( $field_value, array( 'allowed_classes' => false ) );
				if ( is_array( $unserialized ) ) {
					$field_value = $unserialized;
				}
			}

			// Check if we have raw submission value (upload_id => url format)
			if ( is_array( $field_value ) && ! empty( $field_value ) ) {
				$files = array();
				foreach ( $field_value as $upload_id => $file_url ) {
					$files[] = array(
						'data' => array(
							'upload_id' => $upload_id
						)
				...
				// No value available, return empty
				return '';
			}
		}
