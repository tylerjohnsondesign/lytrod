...
class NF_FU_Admin_UploadsTable extends WP_List_Table {
	...
	private function table_data( $form_id = '', $begin_date = '', $end_date = '', $search = '' ) {
		...
		if ( ! empty( $search ) ) {
			...
			$where[] = $wpdb->prepare( '`data` LIKE %s', $search_escaped );
		}
		...
		if ( ! empty( $begin_date ) ) {
			...
			if ( false !== $begin_timestamp ) {
				$begin_date = date( 'Y-m-d 00:00:00', $begin_timestamp );
				$where[]    = $wpdb->prepare( '`date_updated` >= %s', $begin_date );
			}
		}
		...
		if ( ! empty( $end_date ) ) {
			...
			if ( false !== $end_timestamp ) {
				...
				$where[]  = $wpdb->prepare( '`date_updated` <= %s', $end_date );
			}
		}
		...
		foreach ( $uploads as $upload ) {
			$upload_data = unserialize( $upload['data'], array( 'allowed_classes' => false ) );
	...
	private function sort_data( $a, $b ) {
		...
		$sortable_columns = array( 'id', 'filename', 'original_filename', 'date', 'form_name', 'user_name', 'file_location' );
		...
		if ( ! empty( $_GET['order'] ) && in_array( strtolower( $_GET['order'] ), array( 'asc', 'desc' ), true ) ) {
	...
	public static function process_bulk_action() {
		...
		if ( ( isset( $_GET['action'] ) && 'bulk-delete-upload' === $_GET['action'] ) || ( isset( $_GET['action2'] ) && 'bulk-delete-upload' === $_GET['action2'] ) ) {
			...
			check_admin_referer( 'bulk-' . __( 'Uploads', 'ninja-forms-uploads' ) );

			if ( ! isset( $_GET['bulk-delete-upload'] ) || ! is_array( $_GET['bulk-delete-upload'] ) ) {
				return;
			}

			$delete_ids = $_GET['bulk-delete-upload'];
	...
	public function extra_tablenav( $which ) {
		...
		if ( isset( $_REQUEST['form_id'] ) ) {
			$form_selected = absint( $_REQUEST['form_id'] );
		} else {
			$form_selected = 0;
		}

		if ( isset( $_REQUEST['begin_date'] ) ) {
			$begin_date = sanitize_text_field( wp_unslash( $_REQUEST['begin_date'] ) );
		} else {
			$begin_date = '';
		}
