...
class WPOAuth2 {
	 ...
	 * @param string $client_id
	 * @param string $provider
	 * @param bool $returnWpError Optional. Whether to return WP_Error object instead of false on HTTP errors. Default false.
	 *
	 * @return bool|string|\WP_Error Returns access token string on success, false on failure, or WP_Error object if $returnWpError is true and HTTP error occurs.
	 */
	public function refresh_access_token( $client_id, $provider, $returnWpError = false ) {
		$logEntryArray =[
			'timestamp'=>time(),
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( $returnWpError ) {
				return $request; // Return WP_Error object
			}
	...
	/**
	 * Construct URL for disconnect with query args
	 *
	 * @param string $provider
	 * @param string $url
	 * @return void
	...
	/**
	 * Remove provider access token using TokenManager
	 *
	 * @param string $provider
	 * @return void
	 */
	...
	/**
	 * Does the provider have a stored access token?	
	 *
	 * @param string $provider
	 * @return boolean
	 */
