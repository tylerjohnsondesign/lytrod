...
class AdminHandler {
	...
	protected function get_provider_display_name() {
		...
		$name = apply_filters( 'pvw_wp_oauth2_provider_display_name' , $name, $provider );
	...
	public function render_error_notice() {
		...
		$message           = $error_description ? esc_html( $error_description ) : __( 'An unknown error occurred.' );
		$class             = apply_filters( 'pvw_wp_oauth2_error_notice_class', 'error' );
		printf( '<div class="%s"><p><strong>%s %s</strong> &mdash; %s</p></div>', esc_attr( $class ), $provider, esc_html__( 'Connection Error' ), $message );
	}
	...
	public function render_connection_notice() {
		...
		printf( '<div class="%s"><p><strong>%s %s</strong> &mdash; %s</p></div>', esc_attr( $class ), $provider, esc_html__( 'Connected' ), esc_html( $message ) );
	}
	...
	public function render_disconnection_notice() {
		...
		printf( '<div class="%s"><p><strong>%s %s</strong> &mdash; %s</p></div>', esc_attr( $class ), $provider, esc_html__( 'Disconnected' ), esc_html( $message ) );
	}
}