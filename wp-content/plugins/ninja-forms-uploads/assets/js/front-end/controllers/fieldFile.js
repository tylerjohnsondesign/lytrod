(function( $ ) {
	...
	var uploadController = Marionette.Object.extend( {
		...
		cancelUpload: function( e ) {
			...
			// Cleanup is handled by the fileuploadfail event handler on abort
		},
		...
		initFileUpload: function( view ) {
			...
			$file.fileupload( {
				...
				if ( data.errorThrown === 'abort' ) {
					// Upload was cancelled via the Cancel button — perform targeted cleanup.
					// resetError() handles progress bar, cancel button visibility, and active upload tracking.
					self.resetError( view, fieldID );
					return false;
				}
				// Genuine upload failure — clean up tracking and progress display.
				self.removeActiveUpload( formID, fieldID );
				self.resetProgress( e );
			} )
